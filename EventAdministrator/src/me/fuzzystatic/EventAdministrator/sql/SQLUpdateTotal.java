...
public class SQLUpdateTotal extends SQLUpdatePlayer{
	...
	private String updateTotalData(String column, String amount) {
		return "UPDATE " + super.prefix + this.table
				+ " SET " + column + "=" + amount
				+ " WHERE " + SQLSchema.COLUMN_PLAYER_ID + " = " + super.getPlayerID();
	}
    
    private String insertPlayerData(String column, String amount) {
		return "INSERT INTO " + super.prefix + this.table + "(" + SQLSchema.COLUMN_PLAYER_ID + ", " + column + ")"
    ...
    public boolean setTotalData(String column, String amount, boolean update) {
    	...
    	try {
			...
			ConsoleLogs.sendMessage("Error: Could not insert / update player  " + column + " total.");
			e.printStackTrace();
		}
		return false;
	}
    ...
    private String selectTotalData(String column) {
		return "SELECT " + column + " FROM " + super.prefix + this.table
				+ " WHERE " + SQLSchema.COLUMN_PLAYER_ID + " = " + super.getPlayerID();
	}
    ...
    public int getPlayerData(String column) {
    	try {
			...
			ConsoleLogs.sendMessage("Error: Could not acquire player " + column + " data.");
			e.printStackTrace();
		}
		return 0;
    }
    ...
    public boolean setMaxStreakTotalData() {
    	try {
			...
			ConsoleLogs.sendMessage("Error: Could not insert / update player " + SQLSchema.COLUMN_MAXSTREAK + " total.");
			e.printStackTrace();
		}
    	return false;
	}
